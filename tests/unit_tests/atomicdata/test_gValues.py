...
def test_gValue():
    ...
    for i, sp in enumerate(species):
        ...
        for wave in g.keys():
            ax[i].plot(v, g[wave], label=f'K22', color=next(colors))
    ...
    mercury.set_up_geometry_for_model(geometry, mercury.orbperiod)
    
    times = np.linspace(0, mercury.orbperiod.to(u.s).value-1, 1000)
