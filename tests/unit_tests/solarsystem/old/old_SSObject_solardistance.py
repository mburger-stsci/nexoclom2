...
def test_SSObject_solardistance(params):
    ...
    geo_time, obj_time, pos_time = load_object_geometries(objname, center, t_end,
                                                          runtime, 'time')
    # geo_notime, obj_notime = load_object_geometries(objname, center, t_end,
    #                                                 runtime, 'notime')
    
    taa_time = obj_time.taa(times).to(u.deg)
    rsun_time = obj_time.r_sun(times).to(u.au)
    vrsun_time = obj_time.drdt_sun(times).to(u.km/u.s)
    taa_notime = obj_notime.taa(times).to(u.deg)
    rsun_notime = obj_notime.r_sun(times).to(u.au)
    vrsun_notime = obj_notime.drdt_sun(times).to(u.km/u.s)
