...
objnames = 'Mercury', 'Jupiter', 'Saturn', 'Io', 'Moon'
objnames = 'Io', 'Moon'
...
def test_SSObject_solardistance(params):
    ...
    if obj.type == 'Moon':
        horizon_center = load_horizons(obj.orbits, obj.orbperiod*1.5)
        taa_hor = horizon_center['taa']
    #     rsun_hor = horizon_center['r']
    #     vrsun_hor = horizon_center['rdot']
    else:
        taa_hor = horizon['taa']
    rsun_hor = horizon['r']
    ...
    plt.savefig(f'figs_solardistance/distance_and_velocity_{objname}_{center}.png')
    plt.close()
