import numpy as np
import pytest
import hypothesis as hypo
import hypothesis.strategies as st
import astropy.units as u
from nexoclom2.math import Histogram
from nexoclom2.math.ks_test import ks_d, ks_test
from nexoclom2.initial_state import CosThetaAngDist, IsotropicAngDist
import matplotlib.pyplot as plt
from astropy.visualization import quantity_support
quantity_support()



@pytest.mark.initial_state
# @hypo.given(alt0=st.floats(min_value=0, max_value=90),
#             alt1=st.floats(min_value=0, max_value=90),
#             az0=st.floats(min_value=0, max_value=360, exclude_max=True),
#             az1=st.floats(min_value=0, max_value=360))
# @hypo.settings(max_examples=100, deadline=None)
def test_CosThetaAngDist(alt0, alt1, az0, az1):
    
    hypo.assume(((alt0 == alt1) or (alt1-alt0 > 1)) and
                ((az0 == az1) or (np.abs(az0 - az1) > 1)))
    
    params = {'type': 'costheta',
              'azimuth': f'{az0:f}, {az1:f}',
              'altitude': f'{alt0:f}, {alt1:f}',
              'n': '1'}
    angdist = CosThetaAngDist(params)
    isoangdist = IsotropicAngDist(params)
    
    n_packets = 100000
    alt, az = angdist.choose_points(n_packets)
    alt_iso, az_iso = isoangdist.choose_points(n_packets)
    
    hist_az = Histogram(az, bins=np.linspace(0, 360, 361)*u.deg)
    hist_alt = Histogram(alt, bins=np.linspace(0, 90, 91)*u.deg)
    hist_az_iso = Histogram(az_iso, bins=np.linspace(0, 360, 361)*u.deg)
    hist_alt_iso = Histogram(alt_iso, bins=np.linspace(0, 90, 91)*u.deg)
    
    fig, ax = plt.subplots(1, 2, figsize=(12, 6))
    ax[0].plot(hist_az.x, hist_az.histogram, label='CosAngDist')
    ax[0].plot(hist_az_iso.x, hist_az_iso.histogram, label='Isotripic')
    ax[1].plot(hist_alt.x, hist_alt.histogram, label='CosAngDist')
    ax[1].plot(hist_alt_iso.x, hist_alt_iso.histogram, label='Isotripic')
    ax[0].legend()
    plt.pause(1)
    
    from inspect import currentframe, getframeinfo
    frameinfo = getframeinfo(currentframe())
    print(frameinfo.filename, frameinfo.lineno)
    from IPython import embed; embed()
    import sys; sys.exit()
    
    
    if np.isclose(alt0, alt1):
        assert np.allclose(alt, alt0*u.deg)
    else:
        d = ks_d(alt, angdist.cdf_altitude)
        assert d < 3e-2, f'Altitude selection failed. d={d}'
    
    if np.isclose(az0, az1):
        assert np.allclose(az, az0*u.deg)
    else:
        d = ks_d(az, angdist.cdf_azimuth)
        
        assert d < 3e-2, f'Azimuth selection failed. d={d}'
    
    
if __name__ == '__main__':
    test_CosThetaAngDist(0., 90., 0., 360.)
